...
class ValueIterationAgent(ValueEstimationAgent):
  ...
  """
        ...
        for _ in range(self.iterations):
            ...
            for state in mdp.getStates():
                ...
                for action in self.mdp.getPossibleActions(state):
                    Qs.append((action, self.getQValue(state, action)))

                tmpValues[state] = max(Qs, key=lambda Q: Q[1])[1] if len(Qs) else tmpValues[state]
    ...
    """
        ...
        return max(Qs, key=lambda Q: Q[1])[0] if len(Qs) else None
